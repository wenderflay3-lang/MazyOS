const CART_KEY = 'flordelotus_cart';
...
document.addEventListener('DOMContentLoaded', function () {
  ...
  if (checkoutBtn) {
    checkoutBtn.addEventListener('click', function () {
      ...
      let msg = 'Olá! Quero fazer um pedido na Flor de Lótus:\n\n';
      cart.forEach(function (item) {
        msg += '- ' + item.name + ' (' + formatPrice(item.price) + ')\n';
      });
      msg += '\nTotal: ' + formatPrice(cartTotal(cart));
      window.open('https://wa.me/5569981162686?text=' + encodeURIComponent(msg), '_blank');
    });
  }
});
